...
BEGIN
    INSERT INTO store.purchase_order_detail (purchase_order_id, product_id, quantity,total,created_at)
...
BEGIN
    ...
    FROM store.purchase_order_detail det
...
BEGIN
    DELETE FROM store.purchase_order_detail WHERE purchase_order_detail.id = id;
END //
DELIMITER ;
...
BEGIN
    UPDATE store.purchase_order_detail
